...
В отличие от EKF/UKF, FGO переоценивает ВСЕ прошлые состояния при каждой
новой порции данных (полное сглаживание), что даёт лучшую точность на
участках без GNSS.
...
import gtsam
from gtsam import (
    NonlinearFactorGraph, Values, LevenbergMarquardtOptimizer,
    LevenbergMarquardtParams, PreintegratedImuMeasurements,
    ImuFactor, GPSFactor, Pose3, Rot3, Point3, NavState, imuBias,
...
class InsGnssFGO:
    ...
    def __init__(self, lat0_rad, fs_imu,
                 sigma_a_n, sigma_g_n, sigma_ba, sigma_bg,
                 sigma_gps_pos=np.array([0.05, 0.05, 0.10]),
                 init_pose=None, init_vel=np.zeros(3),
                 init_ba=np.zeros(3), init_bg=np.zeros(3)):
        self.dt = 1.0 / fs_imu
        ...
        pose_noise = gtsam.noiseModel.Diagonal.Sigmas(
            np.array([0.02, 0.02, 0.1, 1.0, 1.0, 2.0]))     # rot(3)+pos(3)
